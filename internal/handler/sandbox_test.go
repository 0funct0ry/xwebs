...
func TestDispatcher_Sandbox(t *testing.T) {
	...
	t.Run("Sandbox Enabled - Allowed", func(t *testing.T) {
		d := NewDispatcher(reg, conn, engine, false, nil, nil, true, []string{"echo"}, nil, nil, nil, nil, nil, nil, nil, nil, "")
		h := &Handler{
			Name: "test-allowed",
			Run:  "echo 'hello'",
		}
	...
	t.Run("Sandbox Enabled - Empty Allowlist", func(t *testing.T) {
		d := NewDispatcher(reg, conn, engine, false, nil, nil, true, []string{}, nil, nil, nil, nil, nil, nil, nil, nil, "")
		h := &Handler{
			Name: "test-deny-all",
			Run:  "echo 'forbidden'",
		}
